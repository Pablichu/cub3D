...
/*   Updated: 2021/05/10 15:15:29 by pmira-pe         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static void	init_sprite_info(t_main *main, t_sprite *auxstr, double	*transformY)
{
	...
	hitted_sprite(main, spriteX, spriteY, &auxstr->txt);
...
static void	sprites_print(t_main *main, t_sprite *txt, double *trfY, int strp)
{
	int				y;
	int				texY;
	int				texX;

	while (strp < main->spr.dwEndX)
	{
		texX = (int)(256 * (strp - (-main->spr.sprWth / 2
						+ main->spr.sprScrnX)) * SF / main->spr.sprWth) / 256;
		if (*trfY > 0 && strp > 0 && strp < main->flnf.r[0]
			&& *trfY < main->ray.ZBuffer[strp])
		{
			y = main->spr.dwSrtY;
			while (y < main->spr.dwEndY)
			{
				texY = ((((y - main->spr.vMvScn) * 256 - main->flnf.r[1] * 128
						...
						/ main->spr.sprHht) / 256;
				printing(&main->img, strp, y,
					my_mlx_get_pixel(&main->txt[txt->txt], texX, texY));
				y++;
			}
		}
		strp++;

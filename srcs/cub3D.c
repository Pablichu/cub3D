...
/*   Updated: 2021/04/29 18:59:34 by pmira-pe         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static void	display_results(t_map *flnf)
{
	...
	printf(CYAN "\nSPRITE" RESET " sprite = %s", flnf->txts[4]);
	printf(BLUE "\nFLOAR" RESET " R/%d G/%d B/%d",
		flnf->f[0], flnf->f[1], flnf->f[2]);
	printf("\nCELLING" RESET " R/%d G/%d B/%d"
		YELLOW "Player position |%.0f -- %.0f|\n", flnf->c[0],
		flnf->c[1], flnf->c[2], flnf->ply_pos[0], flnf->ply_pos[1]);
...
static void	args_manage(int ar, char **args, t_map *flnf)
{
	int	i;
...
int	main(int ar, char **args)
{
	...
	if (ar > 1)
	{
		...
		args_manage(ar, args, &main.flnf);
		if (is_bit_set(main.flnf.args_mode, main.flnf.bit.index[3]))
		{
